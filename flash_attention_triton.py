...
def _attn_fwd_inner(acc, l_i, m_i, q,  #
    ...
    # 这个阶段处理的数值范围
    # inner stage=3:  for causal = False
    # inner stage=1:  for causal = True, 先运行
    # inner stage=2:  for causal = True，后运行
    if STAGE == 1:
    ...
    for start_n in range(lo, hi, BLOCK_N):
        ...
        # -- 更新输出累加器 --
        acc = acc * alpha[:, None] #矫正前面的pv的block计算
        # update acc
        # 更新 acc
        ...
        if fp8_v:
            ...
            p = p.to(tl.float16)
        acc = tl.dot(p, v, acc) #pv的block计算，结果累加到acc
        # update m_i and l_i
        # 更新 m_i 和 l_i
        m_i = m_ij
...
def _attn_fwd(Q, K, V, sm_scale, M, Out,  #
    ...
    # grid -> [N_CTX, Z * H, 1] ->(seq_len/block_m, batch*n_heads, 1)
    # Q.shape -> [batch, n_heads, seq_n, head_dim] -> [z, h, seq_n, d]
    start_m = tl.program_id(0) #seq_len/block_m
    off_hz = tl.program_id(1) #batch*n_heads
    off_z = off_hz // H #获取batch索引
    off_h = off_hz % H #获取head索引
    # offset pointers for batch/head，基于 batch/head 的偏移指针
    qvk_offset = off_z.to(tl.int64) * stride_qz + off_h.to(tl.int64) * stride_qh


    # block pointers
    # 块指针
    # 创建一个块指针，包含了基地址、形状、步幅、偏移量和存储顺序等信息。
    # 后续offs_m, offs_n, offs_k等都是基于这个块指针的来访问块数据
    Q_block_ptr = tl.make_block_ptr(
        base=Q + qvk_offset,
        shape=(N_CTX, HEAD_DIM), #(1024, 64)
        strides=(stride_qm, stride_qk), #stride_qm: seq_len, stride_qk: head_dim (64, 1)
        offsets=(start_m * BLOCK_M, 0), #(start_m*64, 0)
        block_shape=(BLOCK_M, HEAD_DIM), #(64, 64)
        order=(1, 0),
    )
    ...
    # For causal = False, STAGE=1, _attn_fwd_inner运行一次_attn_fwd_inner，inner_stage=3, 标准的k,v内循环
    # For causal = True, STAGE=3,  _attn_fwd_inner运行两次，第一次inner_stage=1,第二次inner_stage=2
    if STAGE & 1:
...
class _attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, causal, sm_scale):
        ...
        # q.shape -> [batch, n_heads, seq_n, head_dim] -> [4, 32, 1024, 64]
...
configs = []
for mode in ["fwd", "bwd"]:
    for causal in [True]: # delete true
